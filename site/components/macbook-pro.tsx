...
/**
 * MacBook Pro chrome, from 21st.dev (@designali-in/macbook-pro) — pure SVG, no
 * dependencies. Two changes from the source:
 *
 * - The upstream component paints the screen from an `src` image. We overlay the
 *   live DemoScene into the screen area instead (see `.macbook__screen`), so the
 *   `<image>` slot and its clipPath are dropped rather than left dead.
 * - The camera dot shipped as #080d4c. No blue anywhere in this brand, so it is
 *   a neutral dark now.
 *
 * The screen rect is x=74.52 y=21.32 w=501.22 h=323.85 in a 650x400 viewBox —
 * `.macbook__screen` in globals.css positions the overlay from those numbers.
 */
export function MacbookPro(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 650 400" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <path
        fill="#a4a5a7"
      ...
      />
      <path fill="#3a3a3c" d="M325.11,25.14c-1.99.03-1.99-3.09,0-3.06,1.99-.03,1.99,3.09,0,3.06Z" />
      <polygon
        fill="#b9b9bb"
